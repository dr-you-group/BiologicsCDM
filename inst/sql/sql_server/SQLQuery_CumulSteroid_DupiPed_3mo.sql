...
cohort as (
	...
	where cohort_definition_id = 1440
	), 
Tdrug as (
    ...
    where T5.concept_class_id = 'ingredient' 
          and T1.drug_exposure_end_date >= DATEADD(day, -90, T2.cohort_start_date) 
          and T1.drug_exposure_start_date < DATEADD(day, 90, T2.cohort_start_date)
	), 
Tdate as (
    select SUBJECT_ID, 
           cohort_start_date, 
...
Tcumul as (
    select SUBJECT_ID,
           ...
           ingredient_name,
           sum(case 
                 when drug_exposure_date >= DATEADD(day, -90, cohort_start_date) 
           ...
           sum(case 
                 when drug_exposure_date >= cohort_start_date 
                      and drug_exposure_date < DATEADD(day, 90, cohort_start_date) then quantity
                 else 0 
...
SELECT * INTO @target_database_schema.@target_cohort_table_DupiPed3_temp FROM Tcumul
...
INTO @target_database_schema.@target_cohort_table_DupiPed3
FROM @target_database_schema.@target_cohort_table_DupiPed3_temp
GROUP BY SUBJECT_ID, 
         cohort_start_date;
DROP TABLE @target_database_schema.@target_cohort_table_DupiPed3_temp;